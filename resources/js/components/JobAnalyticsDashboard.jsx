...
const JobAnalyticsDashboard = ({ analytics }) => {
    ...
    // Function to get the refresh data URL
    const getRefreshAnalysisUrl = () => {
        const currentParams = new URLSearchParams(window.location.search);
        if (analytics.search_params?.full_analysis) {
            currentParams.set('full_analysis', '1');
        }
    ...
    // Handle custom message (e.g., when fetching all jobs)
    if (analytics.message) {
        return (
            <div className="p-6 max-w-7xl mx-auto">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
                    <h2 className="text-xl font-semibold text-blue-800 mb-2">Information</h2>
                    <p className="text-blue-700">{analytics.message}</p>
                    <div className="mt-6 flex justify-center">
                        <div className="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest">
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-8">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            ...
                            {analytics.meta && (
                                <p className="text-sm text-blue-500 mt-1">
                                    ...
                                    {analytics.meta.completeness_percentage &&
                                     ` (${analytics.meta.completeness_percentage}% complete)`}
                                </p>
                            )}
                        </div>
                        <div className="flex space-x-3">
                            ...
                            {/* Fetch All Jobs button */}
                            {analytics.meta && analytics.meta.suggest_fetch_all && (
                                <a
                                    href={`${window.location.pathname}?keywords=${encodeURIComponent(analytics.search_params.keywords || '')}&location=${encodeURIComponent(analytics.search_params.location || '')}&fetch_all=1`}
                                    className="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-colors shadow-md"
                                    onClick={() => setIsLoading(true)}
                                >
                                    {isLoading ? (
                                        <>
                            ...
                            <a
                                href={getRefreshAnalysisUrl()}
                                className="px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700 transition-colors shadow-md"
                                onClick={() => setIsLoading(true)}
                            >
                                {isLoading ? (
                                    <>
            ...
            {analytics.meta && (
                <div className="bg-gray-50 p-4 rounded-lg text-center text-sm text-gray-500">
                    ...
                    {analytics.meta.completeness_percentage && (
                        <span> • Analysis is {analytics.meta.completeness_percentage}% complete</span>
                    )}
                </div>
            )}
        </div>
